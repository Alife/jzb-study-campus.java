 ...
 * EXIFExtractor.java
 * 
 * This class based upon code from Jhead, a C program for extracting and manipulating the Exif data within files written by Matthias Wandel. http://www.sentex.net/~mwandel/jhead/
 * 
 * Jhead is public domain software - that is, you can do whatever you want with it, and include it software that is licensed under the GNU or the BSD license, or whatever other licence you choose,
 * including proprietary closed source licenses. Similarly, I release this Java version under the same license, though I do ask that you leave this header in tact.
 * 
 * If you make modifications to this code that you think would benefit the wider community, please send me a copy and I'll post it on my site. Unlike Jhead, this code (as it stands) only supports
 * reading of Exif data - no manipulation, and no thumbnail stuff.
 * 
 ...
 * 
 * Created on 28 April 2002, 23:54 Modified 04 Aug 2002 - Renamed constants to be inline with changes to ExifTagValues interface - Substituted usage of JDK 1.4 features (java.nio package) Modified 29
 * Oct 2002 (v1.2) - Proper traversing of Exif file structure and complete refactor & tidy of the codebase (a few unnoticed bugs removed) - Reads makernote data for 6 families of camera (5 makes) -
 * Tags now stored in directories... use the IFD_ constants to refer to the image file directory you require (Exif, Interop, GPS and Makernote) -- this avoids collisions where two tags share the same
 * code - Takes componentCount of unknown tags into account - Now understands GPS tags (thanks to Colin Briton for his help with this) - Some other bug fixes, pointed out by users around the world.
 * Thanks! Modified 27 Nov 2002 (v2.0) - Renamed to ExifReader - Moved to new package com.drew.metadata.exif Modified since, however changes have not been logged. See release notes for library-wide
 * modifications.
 */
...
/**
 * Extracts Exif data from a JPEG header segment, providing information about the camera/scanner/capture device (if available). Information is encapsulated in an <code>Metadata</code> object.
 * 
...
public class ExifReader implements MetadataReader {
    ...
    private void processMakerNote(int subdirOffset, HashMap processedDirectoryOffsets, int tiffHeaderOffset) {
        ...
        if ("OLYMP".equals(firstFiveChars) || "EPSON".equals(firstFiveChars) || "AGFA".equals(firstFourChars)) {
            ...
            if ("Nikon".equals(firstFiveChars)) {
                /*
                 * There are two scenarios here: Type 1: :0000: 4E 69 6B 6F 6E 00 01 00-05 00 02 00 02 00 06 00 Nikon........... :0010: 00 00 EC 02 00 00 03 00-03 00 01 00 00 00 06 00 ................
                 * Type 3: :0000: 4E 69 6B 6F 6E 00 02 00-00 00 4D 4D 00 2A 00 00 Nikon....MM.... :0010: 00 08 00 1E 00 01 00 07-00 00 00 04 30 32 30 30 ............0200
                 */
    ...
    private void storeThumbnailBytes(ExifDirectory exifDirectory, int tiffHeaderOffset) {
        ...
        if (!exifDirectory.containsTag(ExifDirectory.TAG_THUMBNAIL_LENGTH) || !exifDirectory.containsTag(ExifDirectory.TAG_THUMBNAIL_OFFSET))
            return;
