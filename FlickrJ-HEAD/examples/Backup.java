...
public class Backup {
    ...
    public Backup(String apiKey, String nsid, String sharedSecret, File authsDir)
      throws IOException, ParserConfigurationException {
        this.flickr = new Flickr(apiKey, sharedSecret, new REST(0,5,5));
        this.nsid = nsid;
	...
	private void authorize() throws IOException, SAXException, FlickrException {
		...
		Flickr._trace("Please visit: " + authUrl.toExternalForm() + " then, hit enter.");
		...
		Flickr._trace("Thanks.  You probably will not have to do this every time.  Now starting backup.");
	}
	...
	public void doBackup(File directory) throws Exception {
		...
		while (allIter.hasNext()) {
			...
			while (setIterator.hasNext()) {
				...
				Flickr._trace("Now writing " + filename + " to " + setDirectory.getCanonicalPath());
	...
	public static void main(String[] args) throws Exception {
		if (args.length < 4) {
			Flickr._trace("Usage: java " + Backup.class.getName() + " api_key nsid shared_secret output_dir");
			System.exit(1);
		}
