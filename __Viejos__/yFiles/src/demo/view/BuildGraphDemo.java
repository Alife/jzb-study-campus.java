...
public class BuildGraphDemo extends JPanel 
{
  ...
  void setDefaultGraphics()
  {
    ...
    //set the image
    ivr.setImageURL(getClass().getResource("resource/penguin.gif")); 
    //set node size equals to image size (otherwise scaled)
    ivr.setToImageSize();
    //set the label text
    ivr.setLabelText("Won't bite!"); 
    
    //set the label model to be 8-pos (eight available positions aroiund node)
