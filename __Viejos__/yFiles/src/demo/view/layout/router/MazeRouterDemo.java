 ...
 *     </ul></li>
 * </ol>
 * Additionally this demo shows how uneditable background-layer graphs can be displayed inside
 * the graph view. 
 */
public class MazeRouterDemo extends DemoBase
{
  ...
  public MazeRouterDemo()
  {
    ...
    demoG.getDefaultEdgeRealizer().setTargetArrow(Arrow.STANDARD);
  ...
  private void initializeMaze()
  {
    mazeG = new Graph2D();
    try {
      GMLIOHandler ioHandler = new GMLIOHandler();
      ioHandler.read(mazeG, getClass().getResource("resource/maze.gml"));
    } catch (Exception e){
  ...
  {
    initLnF();
    MazeRouterDemo demo = new MazeRouterDemo();
    demo.start("Orthogonal Edge Router Maze Demo");
  }
  ...
  static class MazeDrawable implements Drawable
  {
    ...
    public Rectangle getBounds(){ return mazeG.getBoundingBox(); }
    public void paint(Graphics2D gfx){ render.paint(gfx, mazeG); }
  }
