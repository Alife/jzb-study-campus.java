 ...
 * port constraint for the selected edge with the radiobutton panel on the left of
 ...
 * <br>
 * The toolbar button "Option..." allows to specify diverse layout paramters 
 ...
 * Portcontraint data gets passed to HierarchicLayouter by binding named DataProviders
 ...
 * In the latter case the inital label information will be automatically 
...
public class HierarchicLayouterDemo extends DemoBase
{
  ...
  PortConstraint getSPC(Edge e)
  {
    ...
    if (pc == null) pc = PortConstraint.create(PortConstraint.ANY_SIDE);
 ...
 void setPC(EdgeMap portMap,EdgeMap groupMap, byte side, boolean strong, Object groupId)
  ...
  {
    initLnF();
    HierarchicLayouterDemo demo = new HierarchicLayouterDemo();
    
    demo.start("Hierarchic Layouter Demo");
  }
    ...
    PortSpec(String title, EdgeMap portMap, EdgeMap groupMap)
    {
      ...
      ActionListener rl = new ActionListener() {
        public void actionPerformed(ActionEvent ev) {
          ...
          } else if ( bm == westB.getModel() ) side = PortConstraint.WEST;
  ...
  void setLabelEdges(boolean labelEdges)
  {
    ...
    if(labelEdges)
    {
      ...
      layoutModule.getOptionHandler().set(HierarchicLayoutModule.EDGE_LABELING,HierarchicLayoutModule.HIERARCHIC);
    }
    else
    {
      ...
      layoutModule.getOptionHandler().set(HierarchicLayoutModule.EDGE_LABELING, HierarchicLayoutModule.NONE);
    }
  ...
  {
    while(er.labelCount() > 0) er.removeLabel(er.getLabel(er.labelCount()-1));
  }
