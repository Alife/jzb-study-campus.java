...
/**
 * This demo shows some {@link NodePlacer}s that are rotatable. The NodePlacers for each node may be changed
 * using the context menu.<br>
 *
 * Some NodePlacers support different {@link RootAlignment}s. Those can be selected using the context
 * menu, too.
 *
 **/
public class RotatableNodePlacersDemo extends AbstractTreeDemo {
  ...
  public static void main( String[] args ) {
    initLnF();
    RotatableNodePlacersDemo demo = new RotatableNodePlacersDemo();
  ...
  public RotatableNodePlacersDemo() {
    ...
    rootAlignmentCombo.setRenderer( new DefaultListCellRenderer() {
      public Component getListCellRendererComponent( JList list, Object value, int index, boolean isSelected, boolean cellHasFocus ) {
        ...
        if ( rootAlignment == RootAlignment.CENTER ) label.setText( "Center" );
        if ( rootAlignment == RootAlignment.LEADING ) label.setText( "Leading" );
        if ( rootAlignment == RootAlignment.LEFT ) label.setText( "Left" );
        if ( rootAlignment == RootAlignment.RIGHT ) label.setText( "Right" );
        if ( rootAlignment == RootAlignment.TRAILING ) label.setText( "Trailing" );
        if ( rootAlignment == RootAlignment.MEDIAN ) label.setText( "Median" );
        if ( rootAlignment == RootAlignment.CENTER_OVER_CHILDREN ) label.setText( "Center over children" );
        return label;
      }
    } );
  ...
  private void changeNodePlacersForSelection() {
    ...
    for ( NodeCursor nodeCursor = view.getGraph2D().selectedNodes(); nodeCursor.ok(); nodeCursor.next() ) {
      Node node = nodeCursor.node();
      AbstractRotatableNodePlacer oldPlacer = ( AbstractRotatableNodePlacer ) nodePlacerMap.get( node );
      Matrix matrix = oldPlacer.getModificationMatrix();
  ...
  private static AbstractRotatableNodePlacer createPlacer( String placerName, Matrix modificationMatrix, RootAlignment rootAlignment ) {
