...
public class IncrementalTreeLayouterDemo extends DemoBase
{
  ...
  private double hDistance = 40;
  private double vDistance = 40;
  ...
  {
    layerStyles.add(new DefaultNodePlacer(DefaultNodePlacer.PLACEMENT_HORIZONTAL_DOWNWARD, DefaultNodePlacer.ALIGNMENT_MEDIAN, 40, 40));
    layerStyles.add(new DefaultNodePlacer(DefaultNodePlacer.PLACEMENT_VERTICAL_TO_RIGHT, DefaultNodePlacer.ALIGNMENT_LEADING_OFFSET, 20, 40));
    layerStyles.add(new DefaultNodePlacer(DefaultNodePlacer.PLACEMENT_HORIZONTAL_DOWNWARD, DefaultNodePlacer.ALIGNMENT_LEADING_OFFSET, DefaultNodePlacer.ROUTING_FORK_AT_ROOT, 10, 20));
    layerStyles.add(new DefaultNodePlacer(DefaultNodePlacer.PLACEMENT_HORIZONTAL_DOWNWARD, DefaultNodePlacer.ALIGNMENT_MEDIAN, 40, 40));
  }
    ...
    spinner.addChangeListener(new ChangeListener()
      {
        public void stateChanged(ChangeEvent ce){
          ...
          while (layer >= layerStyles.size()){
            layerStyles.add(new DefaultNodePlacer(DefaultNodePlacer.PLACEMENT_HORIZONTAL_DOWNWARD, 40, 40));
          }
  ...
  public void calcLayout(){
    if (!view.getGraph2D().isEmpty()){
      if (true) {
        ...
        new BufferedLayouter(treeLayouter).doLayout(view.getGraph2D());
      }
    }
    view.fitContent();
    view.updateView();
  ...
  abstract class AssignLayouterAction extends AbstractAction {
    ...
    public AssignLayouterAction(String name){
      super(name);
    }
  ...
  final class AlignmentAction extends AssignLayouterAction {
    ...
    protected NodePlacer getPlacer(NodePlacer placer)
    {
      if (placer instanceof DefaultNodePlacer){
        ...
        placer = new DefaultNodePlacer(DefaultNodePlacer.PLACEMENT_HORIZONTAL_DOWNWARD, newAlignment, hDistance, vDistance);
      }
      return placer;
    }
  }
  ...
  final class RoutingAction extends AssignLayouterAction {
    ...
    protected NodePlacer getPlacer(NodePlacer placer)
    {
      if (placer instanceof DefaultNodePlacer){
        ...
        placer = new DefaultNodePlacer(DefaultNodePlacer.PLACEMENT_HORIZONTAL_DOWNWARD, DefaultNodePlacer.ALIGNMENT_CENTER, newRouting, hDistance, vDistance);
      }
      return placer;
    }
  }
...
//    }
  ...
  {
    initLnF();
    IncrementalTreeLayouterDemo demo = new IncrementalTreeLayouterDemo();
    demo.start("Incremental Tree Layouter Demo");
  }
}
