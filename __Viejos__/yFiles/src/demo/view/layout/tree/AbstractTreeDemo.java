...
public abstract class AbstractTreeDemo extends DemoBase {
  ...
  protected Color[] layerColors = new Color[]{ Color.red, Color.orange, Color.yellow, Color.blue, Color.cyan, Color.green };
  ...
  protected final class TreeCreateEditMode extends EditMode {
    TreeCreateEditMode() {
      if ( portAssignmentMoveMode == null ) throw new IllegalStateException( "portAssignmentMoveMode is null" );
  ...
  private final class TreeCreateChildEdgeMode extends CreateChildEdgeMode {
    ...
    private void parseNodePlaceMent( Graph2D g, Edge e, EdgeRealizer er ) {
      YPoint firstPoint = er.bendCount() > 0 ? new YPoint( er.firstBend().getX(), er.firstBend().getY() ) : g.getTargetPointAbs( e );
