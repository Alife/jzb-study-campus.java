 ...
 * at the same time.
 * <br>
 * The {@link demo.view.layout.organic.AnimatedNavigationDemo} extends the functionality of this demo
...
public class NavigationDemo extends DemoBase {
  ...
  public static void main( String[] args ) {
    initLnF();
    final NavigationDemo navigationDemo = new NavigationDemo();
    navigationDemo.start( "Navigation Demo" );
  }
   ...
   * It copies the informations from the internal data structure of the layouter to the realizers of the nodes.<br>
   *
  ...
  protected boolean updatePositions() {
    if ( layouter == null || !layouter.isRunning() ) return false;
  ...
  protected void moveToCenter( final Node newCenterNode, boolean animated ) {
    ...
    //Now synchronize the structure updates with the copied graph that is layoutet.
  ...
  protected void setPosition( Node node, double x, double y ) {
    if ( layouter == null || !layouter.isRunning() ) return;
