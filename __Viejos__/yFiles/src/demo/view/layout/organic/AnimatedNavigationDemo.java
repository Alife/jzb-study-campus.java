...
/**
 * This demo is an extension of {@link demo.view.layout.organic.NavigationDemo}.
 * <br>
 * In this demos changing the visible subgraph is now accompanied by animated
 * fade-in and fade-out effects for apprearing and disappearing nodes and edges.
...
public class AnimatedNavigationDemo extends NavigationDemo {
  ...
  protected void moveToCenter( final Node newCenterNode, boolean animated ) {
    ...
    ( ( NavigationDemo )this ).layouter.syncStructure();
    ( ( NavigationDemo )this ).layouter.wakeUp();
  }
  ...
  private void fadeOutNodes(NodeList fadeOutNodes) {
    for (NodeCursor nc = fadeOutNodes.nodes(); nc.ok(); nc.next()) {
      ...
      animationPlayer.animate( factory.fadeOut( nodeDrawable, PREFERRED_DURATION ) ); //<--------                                              FADE OUT
    }
  }
