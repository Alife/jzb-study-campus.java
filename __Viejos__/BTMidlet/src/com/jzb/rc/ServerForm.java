 ...
 * 
 */
public class ServerForm extends Form implements ServerConnectionHandlerListener, CommandListener {
    ...
    ServerForm(MIDletApplication midlet) {
        ...
        append(statusField);
        sendDataField = new TextField("Send data", "Server says: 'Hello, world.'", 64, TextField.ANY);
        ...
        searchCommand = new Command("Search for more", Command.SCREEN, 1);
        addConnectionCommand = new Command("Add connection", Command.SCREEN, 2);
    ...
    public void makeConnections(Vector serviceRecords, int security) {
        for (int i = 0; i < serviceRecords.size(); i++) {
            ServiceRecord serviceRecord = (ServiceRecord) serviceRecords.elementAt(i);
            boolean found = false;
            for (int j = 0; j < handlers.size(); j++) {
                ServerConnectionHandler old = (ServerConnectionHandler) handlers.elementAt(j);
                String oldAddr = old.getServiceRecord().getHostDevice().getBluetoothAddress();
                String newAddr = serviceRecord.getHostDevice().getBluetoothAddress();
            ...
            if (!found) {
                ServerConnectionHandler newHandler = new ServerConnectionHandler(this, serviceRecord, security);
    ...
    void closeAll() {
        for (int i = 0; i < handlers.size(); i++) {
            ServerConnectionHandler handler = (ServerConnectionHandler) handlers.elementAt(i);
            handler.close();
    ...
    public void commandAction(Command cmd, Displayable disp) {
        if (cmd == addConnectionCommand) {
            ...
            for (int i = 0; i < handlers.size(); i++) {
                ServerConnectionHandler handler = (ServerConnectionHandler) handlers.elementAt(i);
                String btAddress = handler.getServiceRecord().getHostDevice().getBluetoothAddress();
            ...
            for (int i = 0; i < handlers.size(); i++) {
                ServerConnectionHandler handler = (ServerConnectionHandler) handlers.elementAt(i);
                try {
                    handler.queueMessageForSending(id, sendData.getBytes());
                    statusField.setText("Queued a send message request");
    ...
    public void handleOpenError(ServerConnectionHandler handler, String errorMessage) {
        statusField.setText("Error opening outbound connection: " + errorMessage);
    }
    ...
    public void handleReceivedMessage(ServerConnectionHandler handler, byte[] messageBytes) {
        ...
        statusField.setText("# messages read: " + numReceivedMessages + " " + "sent: " + numSentMessages);

        // Broadcast message to all clients
        for (int i = 0; i < handlers.size(); i++) {
            ServerConnectionHandler h = (ServerConnectionHandler) handlers.elementAt(i);

            Integer id = new Integer(sendMessageId++);
            try {
                h.queueMessageForSending(id, messageBytes);
            } catch (IllegalArgumentException e) {
                String errorMessage = "IllegalArgumentException while trying to " + "send message: " + e.getMessage();
                handleError(handler, errorMessage);
            }
        }
    }
    ...
    public void handleError(ServerConnectionHandler handler, String errorMessage) {
