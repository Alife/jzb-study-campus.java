...
class ServiceDiscoveryList extends List implements CommandListener, DiscoveryListener, Runnable {
    ...
    ServiceDiscoveryList(MIDletApplication midlet, String uuidString, int inquiryAccessCode) {
        super(TITLE, Choice.IMPLICIT);
        this.midlet = midlet;
        uuid = new UUID(uuidString, false);
        this.inquiryAccessCode = inquiryAccessCode;

        openCommand = new Command("Open connection", Command.SCREEN, 1);
        ...
        try {
            ...
            midlet.serviceDiscoveryListFatalError("Couldn't get a discovery agent: '" + e.getMessage() + "'");
        }
    }
    ...
    public void commandAction(Command command, Displayable d) {
        if (command == logCommand) {
        ...
        } else if (command == searchCommand && !inquiryInProgress && (numServiceSearchesInProgress == 0)) {
            // new inquiry started
            ...
            for (Enumeration keys = matchingServiceRecords.keys(); keys.hasMoreElements();) {
            ...
            try {
                ...
                String iacString = LogScreen.inquiryAccessCodeString(inquiryAccessCode);
                LogScreen.log("startInquiry (" + iacString + ")\n");
                ...
                midlet.serviceDiscoveryListError("Error during startInquiry: '" + e.getMessage() + "'");
            }
            ...
            for (int i = 0; i < size; i++) {
                if (flags[i]) {
                    ...
                    ServiceRecord rec = (ServiceRecord) matchingServiceRecords.get(key);
            ...
            if (selectedServiceRecords.size() > 0) {
                String value = LocalDevice.getProperty("bluetooth.connected.devices.max");
                int maxNum = Integer.parseInt(value);

                int total = numConnectionsAlreadyOpen + selectedServiceRecords.size();
                if (total > maxNum) {
                    midlet.serviceDiscoveryListError("Too many selected. " + "This device can connect to at most " + maxNum + " other devices");
                } else {
                    midlet.serviceDiscoveryListOpen(selectedServiceRecords);
                }
            } else {
                midlet.serviceDiscoveryListError("Select at least one to open");
            }
    ...
    public void deviceDiscovered(RemoteDevice remoteDevice, DeviceClass deviceClass) {
        LogScreen.log("deviceDiscovered: " + remoteDevice.getBluetoothAddress() + " major device class=" + deviceClass.getMajorDeviceClass() + " minor device class="
    ...
    public void inquiryCompleted(int discoveryType) {
        LogScreen.log("inquiryCompleted: " + discoveryType + "\n");
        ...
        if (unsearchedRemoteDevices.size() == 0) {
            ...
            midlet.serviceDiscoveryListError("No Bluetooth devices found");
        }
    }

    public void servicesDiscovered(int transId, ServiceRecord[] serviceRecords) {
        LogScreen.log("servicesDiscovered: transId=" + transId + " # serviceRecords=" + serviceRecords.length + "\n");
        ...
        if (serviceRecords.length == 1) {
            ...
            midlet.serviceDiscoveryListError("Error: Unexpected number (" + serviceRecords.length + ") of service records " + "in servicesDiscovered callback, transId=" + transId);
        }
    }
    ...
    public void serviceSearchCompleted(int transId, int responseCode) {
        ...
        LogScreen.log("serviceSearchCompleted: transId=" + transId + " (" + responseCodeString + ")\n");
        ...
        if (!inquiryInProgress && (transIds.size() == 0)) {
            ...
            if (matchingServiceRecords.size() == 0) {
                midlet.serviceDiscoveryListError("No matching services found");
            }
        }
    }
    ...
    public void doServiceSearch() {
        if ((unsearchedRemoteDevices.size() > 0) && (numServiceSearchesInProgress < sdTransMax)) {
            synchronized (this) {
                ...
                try {
                    ...
                    LogScreen.log("starting service search on device=" + device.getBluetoothAddress() + " transId=" + transId + "\n");
                    ...
                    midlet.serviceDiscoveryListError("Error, could not perform " + "service search: '" + e.getMessage());
                }
            }
        }
    }
