 ...
 * 
 */
public class ClientForm extends Form implements CommandListener, ClientConnectionHandlerListener {
    ...
    public ClientForm(MIDletApplication midlet) {
        ...
        append(statusField);
        sendDataField = new TextField("Send data", "Client says: 'Hello, world.'", 64, TextField.ANY);
    ...
    void closeAll() {
        for (int i = 0; i < handlers.size(); i++) {
            ClientConnectionHandler handler = (ClientConnectionHandler) handlers.elementAt(i);
            handler.close();
        }
    }
    ...
    public void sendMessageToAllClients(int sendMessageId, String sendData) throws IllegalArgumentException {
    ...
    public void handleStreamsOpenError(ClientConnectionHandler handler, String errorMessage) {
        ...
        String str = Integer.toString(handlers.size());
        numConnectionsField.setText(str);
        statusField.setText("Error opening I/O streams: " + errorMessage);
    }
