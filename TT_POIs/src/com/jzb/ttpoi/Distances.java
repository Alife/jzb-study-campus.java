...
public class Distances {
    ...
    private void __calcRoute(DirData info) throws Exception {
        ...
        HTTPConnection conn = new HTTPConnection("maps.googleapis.com");

        NVPair params[] = new NVPair[] { new NVPair("origin", info.originPoint), new NVPair("destination", info.destinationPoint), new NVPair("mode", "driving"), new NVPair("alternatives", "false"),
                new NVPair("units", "metric"), new NVPair("sensor", "false"), new NVPair("language", "es") };

        HTTPResponse res = conn.Get("http://maps.googleapis.com/maps/api/directions/xml", params);
        // System.out.println(new String(res.getData()));

        if (res.getStatusCode() != 200) {
            System.out.println("Error en peticion: ");
            System.out.println(res.getStatusCode());
            System.out.println(res.getReasonLine());
            return;
        }
        ...
        Document doc = null;
        ByteArrayInputStream bais=new ByteArrayInputStream(res.getData());
