...
/**
 * Fujifilm's digicam added the MakerNote tag from the Year2000's model (e.g.Finepix1400, Finepix4700). It uses IFD format and start from ASCII character 'FUJIFILM', and next 4 bytes(value 0x000c)
 * points the offset to first IFD entry. Example of actual data structure is shown below.
 * 
 ...
 * 
 * There are two big differences to the other manufacturers. - Fujifilm's Exif data uses Motorola align, but MakerNote ignores it and uses Intel align. - The other manufacturer's MakerNote counts the
 * "offset to data" from the first byte of TIFF header (same as the other IFD), but Fujifilm counts it from the first byte of MakerNote itself.
 */
