...
/**
 * <p>
 * Base class for {@link Tokenizer} implementations. <code>AbstractTokenizer</code> separates the data analysis from the actual data provision. Although the class maintains read and write positions
 * the physical representation of the logical character buffer behind these positions concerns only the subclasses.
 * </p>
 * 
 * @see Tokenizer
...
public abstract class AbstractTokenizer implements Tokenizer, TokenizerPropertyListener {
     ...
     */
    protected static final int                      VALID_FLAGS_MASK                = Flags.F_RETURN_WHITESPACES | Flags.F_TOKEN_POS_ONLY | Flags.F_KEEP_DATA | Flags.F_COUNT_LINES;
    ...
    /**
     * {@link TokenizerProperties} tha tare used if no others have been specified by calling {@link #setTokenizerProperties}.
     */
    ...
    /**
     * List of currently known token. The first element is the current token returned by the last call to {@link #nextToken}. The following elements are look-ahead token that have already been
     * identified when extracting the current token.
     */
    ...
    /**
     * a combination of <code>F_...</code> constants defined in {@link TokenizerProperties} indicating which bits in the {@link #_flags} member are valid. All other flags are taken from the associated
    ...
    /**
     * Default constructor that sets the tokenizer control flags as it would be approbriate for C/C++ and Java. Found token images are copied. No line nor column informations are provided. Nested
     * comments are not allowed. <br>
     * The tokenizer will use the {@link TokenizerProperties#DEFAULT_WHITESPACES} and {@link TokenizerProperties#DEFAULT_SEPARATORS} for whitespace and separator handling.
     */
    ...
    /**
     * Contructing a <code>AbstractTokenizer</code> with a backing {@link TokenizerProperties} instance.
     * 
     * @param properties
     *            an {@link TokenizerProperties} object containing the settings for the tokenizing process
     */
    ...
    /**
     * Adding an embedded tokenizer. Embedded tokenizer work on the same input buffer as their base tokenizer. A situation where embedded tokenizer could be applied, is a HTML stream with cascading
     * style sheet (CSS) and JavaScript parts. <br>
     * There are no internal means of switching from one tokenizer to another. This should be done by the caller using the method {@link #switchTo}. <br>
     * The {@link TokenizerProperties#F_KEEP_DATA} and {@link TokenizerProperties#F_COUNT_LINES} flags of the base tokenizer take effect also in the embedded tokenizers. <br>
     * Since is might be possible that the given <code>tokenizer</code> is a derivation of the <code>AbstractTokenizer</code> class, this method is synchronized on <code>tokenizer</code>.
     * 
     * @param tokenizer
     *            an embedded tokenizer
     * @throws TokenizerException
     *             if something goes wrong (not likely :-)
     */
    ...
    /**
     * Setting the control flags of the <code>Tokenizer</code>. See the method description in {@link Tokenizer}.
     * 
     * @param flags
    ...
    public void changeParseFlags(int flags, int mask) throws TokenizerException {
        ...
        if ((mask | VALID_FLAGS_MASK) != VALID_FLAGS_MASK) {
            throw new TokenizerException("One or more flags cannot be set separately for a {0}. Violating flags in {1}: {2}.", new Object[] { AbstractTokenizer.class.getName(),
                    Integer.toHexString(flags), Integer.toHexString(mask & ~VALID_FLAGS_MASK) });
        }
    ...
    /**
     * Closing this tokenizer frees resources and deregisters from the associated {@link TokenizerProperties} object.
     */
    @Override
    public void close() {
    ...
    /**
     * Convenience method to retrieve only the token image of the {@link Token} that would be returned by {@link #currentToken}. See the method description in {@link Tokenizer}.
     * 
    ...
    /**
     * Retrieving the number of the currently available characters. See the method description in {@link Tokenizer}.
     * 
    ...
    /**
     * Retrieve the {@link Token} that was found by the last call to {@link #nextToken}. See the method description in {@link Tokenizer}.
     * 
     ...
     * 
     * @param pos
     *            get character on this position in the data stream
    ...
    /**
     * If the flag {@link TokenizerProperties#F_COUNT_LINES} is set, this method will return the current column positionstarting with 0 in the input stream. See the method description in
     * {@link Tokenizer}.
     * 
    ...
    /**
     * Retrieve the current column. The method can only be used if the flag <code>F_COUNT_LINES</code> has been set. Without this flag being set, the return value is undefined. Note that column
     * counting starts with 0, while editors often use 1 for the first column in one row.
     * 
    ...
    /**
     * Query the current row. The method can only be used if the flag {@link TokenizerProperties#F_COUNT_LINES} has been set. Without this flag being set, the return value is undefined. <br>
     * Note that row counting starts with 0, while editors often use 1 for the first row.
     * 
     * @return current row (starting with 0) or -1 if the flag {@link TokenizerProperties#F_COUNT_LINES} is set
     */
    ...
    /**
     * Retrieving the current {@link de.susebox.jtopas.spi.KeywordHandler}. See the method description in {@link Tokenizer}.
     * 
     * @return the currently active whitespace keyword or <code>null</code>, if keyword support is switched off
     */
    @Override
    ...
    /**
     * If the flag {@link TokenizerProperties#F_COUNT_LINES} is set, this method will return the line number starting with 0 in the input stream. See the method description in {@link Tokenizer}.
     * 
    ...
    /**
     * Retrieving the parser control flags. See the method description in {@link Tokenizer}.
     * 
    ...
    /**
     * Retrieving the current {@link de.susebox.jtopas.spi.PatternHandler}. See the method description in {@link Tokenizer}.
     * 
    ...
    /**
     * Retrieving the current {@link de.susebox.jtopas.spi.SeparatorHandler}. See the method description in {@link Tokenizer}.
     * 
     * @return the currently active {@link SeparatorHandler} or <code>null</code>, if separators aren't recognized by the tokenizer
    ...
    /**
     * Retrieving the current {@link SequenceHandler}. See the method description in {@link Tokenizer}.
     * 
     * @return the currently active {@link SequenceHandler} or null, if the base implementation is working
     */
    @Override
    ...
    /**
     * Retrieving the {@link TokenizerSource} of this <code>Tokenizer</code>. The method may return <code>null</code> if there is no <code>TokenizerSource</code> associated with it.
     * 
     * @param the
     *            {@link TokenizerSource} associated with this <code>Tokenizer</code>
     * @see #setSource
     */
    @Override
    ...
    /**
     * Retrieve text from the currently available range. See the method description in {@link Tokenizer}.
     * 
     * @param start
    ...
    /**
     * Retrieving the current tokenizer characteristics. See the method description in {@link Tokenizer}.
     * 
    ...
    /**
     * Retrieving the current {@link de.susebox.jtopas.spi.WhitespaceHandler}. See the method description in {@link Tokenizer}.
     * 
    ...
    /**
     * Checking if there are more tokens available. See the method description in {@link Tokenizer}.
     * 
     * @return <code>true</code> if a ca_ll to {@link #nextToken} or {@link #nextImage} will succed, <code>false</code> otherwise
     */
    @Override
    ...
    /**
     * This method is a convenience method. It returns only the next token image without any informations about its type or associated information. See the method description in {@link Tokenizer}.
     * 
    ...
    public Token nextToken() throws TokenizerException {
        ...
        __MAIN_LOOP__: do {
            ...
            if (returnIt) {
                boolean tokenPosOnly = (prop != null) ? isFlagSet(prop, Flags.F_TOKEN_POS_ONLY) : isFlagSet(Flags.F_TOKEN_POS_ONLY);
                boolean returnImageParts = (prop != null) ? isFlagSet(prop, Flags.F_RETURN_IMAGE_PARTS) : isFlagSet(Flags.F_RETURN_IMAGE_PARTS);
    ...
    /**
     * Event handler method. The given {@link TokenizerPropertyEvent} parameter contains the nessecary information about the property change. We choose one single method in favour of various more
     * specialized methods since the reactions on adding, removing and modifying tokenizer properties are often the same (flushing cash, rereading information etc.) are probably not very different. <br>
     * Note that a modification of the parse flags in the backing {@link TokenizerProperties} object removes all flags previously modified through {@link #changeParseFlags}.
     * 
     * @param event
     *            the {@link TokenizerPropertyEvent} that describes the change
     */
    @Override
            ...
            switch (event.getType()) {
                ...
                case TokenizerPropertyEvent.PROPERTY_REMOVED:
                    switch (prop.getType()) {
                        ...
                        case Token.STRING:
                        case Token.SPECIAL_SEQUENCE:
                            if ((_internalFlags & IFLAG_EXTERNAL_SEQUENCE_HANDLER) == 0 && _properties instanceof de.susebox.jtopas.spi.SequenceHandler) {
                                setSequenceHandler((de.susebox.jtopas.spi.SequenceHandler) _properties);
                            }
                            break;
                        case Token.KEYWORD:
                            if ((_internalFlags & IFLAG_EXTERNAL_KEYWORD_HANDLER) == 0 && _properties instanceof de.susebox.jtopas.spi.KeywordHandler) {
                                setKeywordHandler((de.susebox.jtopas.spi.KeywordHandler) _properties);
                            }
                            break;
                        case Token.PATTERN:
                            if ((_internalFlags & IFLAG_EXTERNAL_PATTERN_HANDLER) == 0 && _properties instanceof de.susebox.jtopas.spi.PatternHandler) {
    ...
    /**
     * Try to read more data into the text buffer of the tokenizer. See the method description in {@link Tokenizer}.
     * 
    ...
    /**
     * Setting a new {@link de.susebox.jtopas.spi.KeywordHandler} or removing any previously installed one. See the method description in {@link Tokenizer}.
     * 
     * @param handler
     *            the (new) {@link KeywordHandler} to use or <code>null</code> to remove it
     */
    @Override
    ...
    /**
     * Setting a new {@link de.susebox.jtopas.spi.PatternHandler} or removing any previously installed one. See the method description in {@link Tokenizer}.
     * 
     * @param handler
    ...
    /**
     * This method sets the tokenizers current read position to the given absolute read position. See the method description in {@link Tokenizer}. <br>
     * When using this method with embedded tokenizers, the user is responsible to set the read position in the currently used tokenizer. It will be propagated by the next call to {@link #switchTo}.
     * Until that point, a call to this method has no effect on the other tokenizers sharing the same data source.
     * 
     * @param position
    ...
    public void setReadPositionAbsolute(int position) throws IndexOutOfBoundsException {
        if (position < getRangeStart()) {
            throw new ExtIndexOutOfBoundsException("Invalid read position {0} below the current text window start {1}.", new Object[] { new Integer(position), new Integer(getRangeStart()) });
        } else if (position > _currentWritePos) {
            throw new ExtIndexOutOfBoundsException("Invalid read position {0} at or above the current text window end {1}.", new Object[] { new Integer(position),
                    new Integer(currentlyAvailable() + getRangeStart()) });
        }
    ...
    /**
     * This method sets the tokenizers new read position the given number of characters forward (positive value) or backward (negative value) starting from the current read position. See the method
     * description in {@link Tokenizer}. <br>
     * When using this method with embedded tokenizers, the user is responsible to set the read position in the currently used tokenizer. It will be propagated by the next call to {@link #switchTo}.
     * Until that point, a call to this method has no effect on the other tokenizers sharing the same data source.
     * 
     * @param offset
    ...
    /**
     * Setting a new {@link de.susebox.jtopas.spi.SeparatorHandler} or removing any previously installed <code>SeparatorHandler</code>. See the method description in {@link Tokenizer}.
     * 
     * @param handler
    ...
    /**
     * Setting a new {@link de.susebox.jtopas.spi.SequenceHandler} or removing any previously installed one. See the method description in {@link Tokenizer}.
     * 
     * @param handler
     *            the (new) {@link SequenceHandler} to use or null to remove it
     */
    @Override
    public void setSequenceHandler(de.susebox.jtopas.spi.SequenceHandler handler) {
        synchronized (this) {
            if (handler == _properties) {
                if (_properties != null
                        && (_properties.getSpecialSequences().hasNext() || _properties.getStrings().hasNext() || _properties.getBlockComments().hasNext() || _properties.getLineComments().hasNext())) {
    ...
    /**
     * Convenience method to avoid the construction of a {@link TokenizerSource} from the most important data source {@link java.io.Reader}.
     * 
     * @param reader
     *            the {@link java.io.Reader} to get data from
     */
     ...
     * Subclasses should override this method to do their own actions on a data source change. Generally, this base method should be called first in the subclass implementation of
     * <code>setSource</code> (equivalent to super calls in constructors of derived classes).
     * 
     * @param source
    ...
    /**
     * Setting the tokenizer characteristics. See the method description in {@link Tokenizer}.
     * 
     * @param props
    ...
    public void setTokenizerProperties(TokenizerProperties props) throws NullPointerException {
        ...
        if (newFlags != _flags) {
            propertyChanged(new TokenizerPropertyEvent(TokenizerPropertyEvent.PROPERTY_MODIFIED, new TokenizerProperty(TokenizerProperty.PARSE_FLAG_MASK,
                    new String[] { Integer.toBinaryString(newFlags) }), new TokenizerProperty(TokenizerProperty.PARSE_FLAG_MASK, new String[] { Integer.toBinaryString(_flags) })));
        }
    }

    /**
     * Setting a new {@link de.susebox.jtopas.spi.WhitespaceHandler} or removing any previously installed one. See the method description in {@link Tokenizer}.
     * 
     * @param handler
    ...
    /**
     * Changing fron one tokenizer to another. If the given tokenizer has not been added with {@link #addTokenizer}, an exception is thrown.<br>
     * The <code>switchTo</code> method does the nessecary synchronisation between <code>this</code> and the given tokenizer. The user is therefore responsible to use <code>switchTo</code> whenever a
     * tokenizer change is nessecary. It must be done this way: <blockquote>
     * 
     * <pre>
     ...
     * </pre>
     * 
     * </blockquote> That way we avoid a more complex synchronisation between tokenizers whenever one of them parses the next data in the input stream. However, the danger of not synchronized
     * tokenizers remains, so take care. <br>
     * Since is might be possible that the given <code>tokenizer</code> is a derivation of the <code>AbstractTokenizer</code> class, this method is synchronized on <code>tokenizer</code>.
     * 
     * @param tokenizer
     *            the tokenizer that should be used from now on
     */
    ...
    /**
     * The method recomputes the line and column position of the tokenizer, if the flag {@link TokenizerProperties#F_COUNT_LINES} is set. It gets the token type of the {@link Token} that has been
     * retrieved by the calling {@link #nextToken}. Using the tokenizer control flags and certain other information it tries to to find end-of-line sequences as fast as possible. For example, a line
     * comment should always contain a end-of-line sequence, so we can simply increase the line count and set the column count to 0.
     * 
     * @param type
     ...
     * 
     * @param offset
     *            start comparing at this offset from the current read position
     * @param prefic
     *            compare read data with this prefix
     * @param noCase
     *            case- or not case-sensitive comparison
     * @throws TokenizerException
     *             failure while reading data from the input stream
     * @return 0 if the the given prefix matches the input stream, -1 on EOF and 1 if not matching
     */
    protected int comparePrefix(int offset, String prefix, boolean noCase) throws TokenizerException {
        // compare
        ...
        for (int pos = offset; pos < offset + len; ++pos) {
            ...
            if (c1 != c2 && (!noCase || Character.toUpperCase(c1) != Character.toUpperCase(c2))) {
                return 1;
            }
        }
    ...
    /**
     * Completing a block comment. After a block comment sequence has been found, all characters up to and including the end sequence of the block comment belong to the block comment. Note that on
     * reaching end-of-file a block comment does not nessecarily ends with an end-of-block-comment sequence.
     * 
     * @param prop
     *            the property describing the block comment to complete
     ...
     * 
     * @param token
     *            buffer to receive information about the keyword or normal token
    ...
    protected TokenizerProperty completeBoundedToken(Token token) throws TokenizerException {
        ...
        while (!(isEOF(len) || isWhitespace(len) || isPattern(len, true) || isSpecialSequence(len) || isSeparator(len))) {
            len++;
        }
        token.setLength(len);
    ...
    /**
     * Completing a line comment. After a line comment sequence has been found, all characters up to and including the end-of-line combination belong to the line comment. Note that on reaching
     * end-of-file a line comment does not nessecarily ends with an end-of-line sequence (linefeed for example).
     * 
     * @param prop
     *            the property describing the line comment to complete
    ...
    /**
     * Completing a string. After a string start sequence has been found, all characters up to and including the end-of-string sequence belong to the string. Note that on reaching end-of-file a string
     * does not nessecarily ends with an end-of-string sequence.
     * 
     * @param prop
     *            the property describing the string to complete
    ...
    protected int completeString(TokenizerProperty prop) throws TokenizerException {
        ...
        int len = start.length();
        boolean noCase = isFlagSet(prop, Flags.F_NO_CASE);
        boolean escEqualsEnd = (!noCase && esc.compareTo(end) == 0) || (noCase && esc.compareToIgnoreCase(end) == 0);
    ...
    /**
     * Returns the {@link de.susebox.jtopas.spi.DataProvider} of the base tokenizer. This is this tokenizer if it is not an embedded one.
     * 
     * @param startPos
    ...
    /**
     * Subclasses have to provide {@link de.susebox.jtopas.spi.DataProvider} instances for various token type handlers. The given start position is the absolute number of characters from the beginning
     * of the data source.
     * 
     * @param startPos
     *            position in the input data
     ...
     * 
     * @param offset
     *            check at this position relative to the current read position
    ...
    /**
     * Checking a given flag. The method considers both the globally set flags in the associated {@link TokenizerProperties} instance and the locally set by {@link #changeParseFlags}.
     * 
     * @param flag
     *            one of the <code>F_...</code> flags defined in {@link TokenizerProperties}
     */
    ...
    /**
     * Checking if a given flag is set for the given {@link TokenizerProperty}, for this <code>Tokenizer</code> or for the used {@link TokenizerProperties}. The method considers both the globally set
     * flags in the associated {@link TokenizerProperties} instance and the locally set by {@link #changeParseFlags}.
     * 
     * @param prop
     *            check the flag for this property
     * @param flag
     *            one of the {@link Flags} constants
     */
    ...
    /**
     * This method checks if the character sequence starting at a given position with a given lenghth is a keyword. If so, it returns the keyword description as {@link TokenizerProperty} object.
     * 
     * @param startingAtPos
     *            check at this position
     * @param length
     *            the candidate has this number of characters
     * @throws TokenizerException
     *             routed exception from the active {@link de.susebox.jtopas.spi.KeywordHandler}
     ...
     * @param offset
     *            check at this position relative to the current read position
     * @param freePatternOnly
     *            if <code>true</code> consider only pattern that can occur anywhere in the data
     * @throws TokenizerException
     *             failure while reading data from the input stream
     * @return <code>true</code> if a pattern match was found at the given offset, <code>false</code> otherwise
     */
        ...
        if (_patternHandler != null) {
            ...
            if (result != null && (isFree || !freePatternOnly)) {
                if (!isFree) {
                    ...
                    if (isEOF(nextOffset) || isWhitespace(nextOffset) || isPattern(nextOffset, true) || isSpecialSequence(nextOffset) || isSeparator(nextOffset)) {
     ...
     * @throws TokenizerException
     *             failure while reading data from the input stream
     * @return <code>true</code> if a separator was found atthe given offset, <code>false</code> otherwise
     */
    protected boolean isSeparator(int offset) throws TokenizerException {
        if (_separatorHandler != null && (_currentReadPos + offset < _currentWritePos || readMoreDataFromBase() > 0) && _separatorHandler.isSeparator(getChar(_currentReadPos + offset))) {
    ...
    /**
     * This method checks at the given offset if it contains a a special sequence. Unlike the method {@link #test4SpecialSequence} it does nothing more.
     * 
     * @param offset
     *            check at this position relative to the current read position
     * @throws TokenizerException
     *             failure while reading data from the input stream
     * @return <code>true</code> if a special sequence was found at the given offset, <code>false</code> otherwise
     */
     ...
     * @return <code>true</code> if the given character is a whitespace, <code>false</code> otherwise
     */
     ...
     * @throws TokenizerException
     *             failure while reading data from the input stream
     * @return <code>true</code> if a whitespace sequence was found at the given offset, <code>false</code> otherwise
     */
    ...
    /**
     * This method is called when the tokenizer runs out of data. Its main purpose is to call the {@link TokenizerSource#read} method. It is also responsible to handle the flag
     * {@link TokenizerProperties#F_KEEP_DATA} flag).
     * 
     * @return number of read bytes or -1 if an end-of-file condition occured
     * @throws TokenizerException
     *             wrapped exceptions from the {@link TokenizerSource#read} method
     */
    protected abstract int readMoreData() throws TokenizerException;

    /**
     * This method organizes the input buffer. It moves the current text window if nessecary or allocates more space, if data should be kept completely (see the {@link TokenizerProperties#F_KEEP_DATA}
     * flag). Its main purpose is to call the {@link TokenizerSource#read} method.
     * 
     * @return number of read bytes or -1 if an end-of-file condition occured
     * @throws TokenizerException
     *             wrapped exceptions from the {@link TokenizerSource#read} method
    ...
    /**
     * This method detects the number of whitespace characters starting at the given position. It should return the number of characters identified as whitespaces starting from and including the given
     * start position. <br>
     * Then overriding this method, use {@link #getBaseDataProvider} to access characters. <br>
     ...
     * 
     * @param startingAtPos
     *            start checking for whitespace from this position
     * @param maxChars
     *            if there is no non-whitespace character, read up to this number of characters
     ...
     * 
     * @param prop
     *            the {@link TokenizerProperty} describing a block comment
     * @param image
     *            split this string into lines
     * @return an array containing the lines of the image without line separator characters
     */
    protected String[] splitBlockComment(TokenizerProperty prop, String image) {
        ...
        if (image.length() - start.length() >= end.length()
                && ((!noCase && end.equals(image.substring(image.length() - end.length()))) || (noCase && end.equalsIgnoreCase(image.substring(image.length() - end.length()))))) {
     ...
     * 
     * @param prop
     *            the {@link TokenizerProperty} describing a string
     * @param image
     *            split this string into lines
     * @return an array containing the lines of the image without line separator characters
     */
    protected String[] splitString(TokenizerProperty prop, String image) {
        ...
        while (index < endIndex) {
            if ((!noCase && image.startsWith(esc, index)) || (noCase && image.substring(index, index + esc.length()).equalsIgnoreCase(esc))) {
                if (buffer == null) {
                ...
                if (index < image.length()) {
                    if ((!noCase && image.startsWith(esc, index)) || (noCase && image.substring(index, index + esc.length()).equalsIgnoreCase(esc))) {
                        buffer.append(esc);
                        index += esc.length();
                    } else if ((!noCase && image.startsWith(begin, index)) || (noCase && image.substring(index, index + begin.length()).equalsIgnoreCase(begin))) {
                        buffer.append(begin);
                        index += begin.length();
                    } else if ((!noCase && image.startsWith(end, index)) || (noCase && image.substring(index, index + end.length()).equalsIgnoreCase(end))) {
                        buffer.append(end);
                        index += end.length();
                    }
                }
    ...
    /**
     * When the method {@link #readMoreData} changes the contents of the input buffer or the input buffer itself, all embedded tokenizers must be synchronized. That means their member variables are
     * adjusted to the base tokenizer.
     * 
     * @throws TokenizerException
     ...
     * 
     * @param position
     *            the position to map to the current line number
     */
