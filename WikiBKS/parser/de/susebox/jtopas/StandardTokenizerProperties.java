...
/**
 * <p>
 * The class <code>StandardTokenizerProperties</code> provides a simple implementation of the {@link TokenizerProperties} interface for use in most situations.
 * </p>
 * <p>
...
public class StandardTokenizerProperties extends AbstractTokenizerProperties implements TokenizerProperties, DataMapper {
    ...
    /**
     * Maximum length of a non-free pattern match. These are patterns that dont have the {@link TokenizerProperties#F_FREE_PATTERN} flag set. A common example are number patterns.
     */
    ...
    /**
     * Like the array {@link #_sequences} this two-element Array contains two {@link de.susebox.jtopas.impl.SequenceStore}, the first for the case-sensitive keywords, the second for the
    ...
    /**
     * The first element is the {@link de.susebox.jtopas.impl.SequenceStore} for the case-sensitive sequences, the second is for the case-insensitive ones.
     */
    ...
    /**
     * Default constructor that intitializes an instance with the default whitespaces and separator sets. {@link Tokenizer} instances using this <code>StandardTokenizerProperties</code> object, split
     * text between spaces, tabs and line ending sequences as well as between punctuation characters.
     */
     ...
     * </pre>
     * 
     * See the {@link TokenizerProperties} interface for the supported flags. <br>
     * The {@link TokenizerProperties#DEFAULT_WHITESPACES} and {@link TokenizerProperties#DEFAULT_SEPARATORS} are used for whitespace and separator handling if no explicit calls to
    ...
    /**
     * This method detects the number of whitespace characters the data range given through the {@link DataProvider} parameter starts with.
     * 
     * @param dataProvider
     *            the source to get the data range from
     ...
     *             if no {@link DataProvider} is given
    ...
    /**
     * This method returns an {@link java.util.Iterator} of {@link TokenizerProperty} objects. See the method description in {@link TokenizerProperties}.
     * 
    ...
    /**
     * Obtaining the separator set of the <code>Tokenizer</code>. See the method description in {@link TokenizerProperties}.
     * 
     * @see #setSeparators
    ...
    /**
     * This method returns the length of the longest special sequence, comment or string prefix that is known to this <code>SequenceHandler</code>. When calling
     * {@link #startsWithSequenceCommentOrString}, the passed {@link DataProvider} parameter will supply at least this number of characters (see {@link DataProvider#getLength}). If less characters are
     * provided, EOF is reached.
     * 
     * @return the number of characters needed in the worst case to identify a special sequence
     */
    @Override
    ...
    /**
     * The method retrieves the backing {@link de.susebox.jtopas.TokenizerProperties} instance, this <code>DataMapper</code> is working on. For implementations of the <code>TokenizerProperties</code>
     * interface that also implement the <code>DataMapper</code> interface, this method returns the instance itself it is called on. <br>
     * Otherwise the method returns the <code>TokenizerProperties</code> instance passed through the last call to {@link #setTokenizerProperties} or <code>null</code> if no such call has taken place
     * so far.
     * 
    ...
    /**
     * Obtaining the whitespace character set. See the method description in {@link TokenizerProperties}.
     * 
     * @see #setWhitespaces
    ...
    /**
     * This method can be used by a {@link de.susebox.jtopas.Tokenizer} implementation for a fast detection if keyword matching must be performed at all. If the method returns <code>false</code>
     * time-consuming preparations can be skipped.
     * 
     * @return <code>true</code> if there actually are pattern that can be tested for a match, <code>false</code> otherwise.
     */
    @Override
    ...
    /**
     * This method can be used by a {@link de.susebox.jtopas.Tokenizer} implementation for a fast detection if pattern matching must be performed at all. If the method returns <code>false</code>
     * time-consuming preparations can be skipped.
     * 
     * @return <code>true</code> if there actually are pattern that can be tested for a match, <code>false</code> otherwise.
     */
    ...
    /**
     * This method can be used by a {@link de.susebox.jtopas.Tokenizer} implementation for a fast detection if special sequence checking must be performed at all. If the method returns
     * <code>false</code> time-consuming preparations can be skipped.
     * 
     * @return <code>true</code> if there actually are pattern that can be tested for a match, <code>false</code> otherwise.
     */
    @Override
    ...
    /**
     * This method checks if the character range given through the {@link DataProvider} comprises a keyword.
     * 
     * @param dataProvider
     *            the source to get the data from, that are checked
     * @return a {@link de.susebox.jtopas.TokenizerProperty} if a keyword could be found, <code>null</code> otherwise
     * @throws TokenizerException
     *             failure while reading more data
     * @throws NullPointerException
     *             if no {@link DataProvider} is given
     */
    @Override
     ...
     * @return <code>true</code> if the given character is a separator, <code>false</code> otherwise
     */
    @Override
    ...
    /**
     * This method checks if the start of a character range given through the {@link DataProvider} matches a pattern.
     * 
     * @param dataProvider
     *            the source to get the data from
     * @return a {@link PatternHandler.Result} object or <code>null</code> if no match was found
     * @throws TokenizerException
     *             generic exception
     * @throws NullPointerException
     *             if no {@link DataProvider} is given
    ...
    /**
     * If a {@link Tokenizer} performs line counting, it is often nessecary to know if newline characters is considered to be a whitespace. See {@link WhitespaceHandler} for details.
     * 
     * @return <code>true</code> if newline characters are in the current whitespace set, <code>false</code> otherwise
     * 
     */
    @Override
    public boolean newlineIsWhitespace() {
        return (_charFlags['\n'] & CHARFLAG_WHITESPACE) != 0 && (_charFlags['\r'] & CHARFLAG_WHITESPACE) != 0;
    }
    ...
    /**
     * Setting the backing {@link TokenizerProperties} instance this <code>DataMapper</code> is working with. Usually, the <code>DataMapper</code> interface is implemented by
     * <code>TokenizerProperties</code> implementations, too. Otherwise the {@link Tokenizer} using the <code>TokenizerProperties</code>, will construct a default <code>DataMapper</code> an propagate
     * the <code>TokenizerProperties</code> instance by calling this method. <br>
     * The method should throw an {@link java.lang.UnsupportedOperationException} if this <code>DataMapper</code> is an extension to an <code>TokenizerProperties</code> implementation.
     * 
     * @param props
     *            the {@link de.susebox.jtopas.TokenizerProperties}
     * @throws UnsupportedOperationException
     *             is this is a <code>DataMapper</code> implemented by a {@link de.susebox.jtopas.TokenizerProperties} implementation
     * @throws NullPointerException
     *             if no {@link TokenizerProperties} are given
     */
    @Override
    public void setTokenizerProperties(TokenizerProperties props) throws UnsupportedOperationException, NullPointerException {
        throw new ExtUnsupportedOperationException("Class {0} already defines the {1} interface.", new Object[] { StandardTokenizerProperties.class.getName(), DataMapper.class.getName() });
    }

    /**
     * This method checks if a given range of data starts with a special sequence, a comment or a string. These three types of token are testet together since both comment and string prefixes are
     * ordinary special sequences. Only the actions preformed <strong>after</strong> a string or comment has been detected, are different. <br>
     * The method returns <code>null</code> if no special sequence, comment or string could matches the the leading part of the data range given through the {@link DataProvider}. <br>
     * In cases of strings or comments, the return value contains the description for the introducing character sequence, <strong>NOT</strong> the whole string or comment. The reading of the rest of
     * the string or comment is done by the calling {@link de.susebox.jtopas.Tokenizer}.
     * 
     * @param dataProvider
     *            the source to get the data range from
     * @return a {@link de.susebox.jtopas.TokenizerProperty} if a special sequence, comment or string could be detected, <code>null</code> otherwise
     * @throws TokenizerException
     *             failure while reading more data
     * @throws NullPointerException
     *             if no {@link DataProvider} is given
    ...
    public TokenizerProperty startsWithSequenceCommentOrString(DataProvider dataProvider) throws TokenizerException, NullPointerException {
        ...
        synchronized (_sequences) {
            TokenizerProperty caseProp = (_sequences[0] != null) ? _sequences[0].startsWithSequenceCommentOrString(dataProvider) : null;

            TokenizerProperty noCaseProp = (_sequences[1] != null) ? _sequences[1].startsWithSequenceCommentOrString(dataProvider) : null;
    ...
    /**
     * Registering a keyword property. The method assumes that the given keyword property has been checked for not being null, having a non-empty keyword image and normalized flags (
     * {@link AbstractTokenizerProperties#normalizeFlags}).
     * 
     * @param keywordProp
     *            keyword property to register
    ...
    /**
     * Registering a pattern with an associated object. The method assumes that the given pattern property has been checked for not being null, having a non-empty pattern image and normalized flags (
     * {@link AbstractTokenizerProperties#normalizeFlags}). See the method description in {@link AbstractTokenizerProperties}.
     * 
     * @param patternProp
     *            the regular expression to be added
     * @return the replaced pattern property or <code>null</code>
     * @throws IllegalArgumentException
     *             if pattern matching is not available
     */
    ...
    /**
     * This method adds or replaces strings, comments and ordinary special sequences. The method assumes that the given special sequence property has been checked for not being null, having a
     * non-empty imagesand normalized flags ({@link AbstractTokenizerProperties#normalizeFlags}).
     * 
     * @param property
     *            the description of the new sequence
    ...
    /**
     * Registering a {@link TokenizerProperty}. See the method description in {@link AbstractTokenizerProperties}.
     * 
     * @param property
    ...
    protected TokenizerProperty doAddProperty(TokenizerProperty property) {
        switch (property.getType()) {
            ...
            default:
                throw new ExtIllegalArgumentException("Unsupported property type {0}. (Leading) image \"{1}\".", new Object[] { new Integer(property.getType()), property.getImages()[0] });
        }
    }

    /**
     * Retrieving a property by a given type and image. See the method description in {@link AbstractTokenizerProperties} for details.
     * 
     * @param type
    ...
    protected TokenizerProperty doGetProperty(int type, String startImage) {
        ...
        switch (type) {
            ...
            case Token.SEPARATOR:
            default:
                throw new ExtIllegalArgumentException("Unsupported property type {0}. (Leading) image \"{1}\".", new Object[] { new Integer(type), startImage });
        }
    ...
    /**
     * Deregistering a {@link TokenizerProperty} from the store. See the method description in {@link AbstractTokenizerProperties}.
     * 
     * @param property
     *            property to remove
     * @return the replaced property or <code>null</code>
    ...
    protected TokenizerProperty doRemoveProperty(TokenizerProperty property) {
        ...
        switch (property.getType()) {
            ...
            case Token.SEPARATOR:
            default:
                throw new ExtIllegalArgumentException("Unsupported property type {0}. (Leading) image \"{1}\".", new Object[] { new Integer(property.getType()), image });
        }
    ...
    /**
     * Setting a new separator set. See the method description in {@link AbstractTokenizerProperties} for details.
     * 
     * @param separators
     *            the set of separators including ranges
    ...
    /**
     * Setting a new whitespace set. See the method description in {@link AbstractTokenizerProperties} for details.
     * 
     * @param whitespaces
     *            the set of whitespaces including ranges
    ...
    /**
     * Set or removes the flags corresponding to type and case-sensitivity from the character flags tables.
     * 
     * @param set
     *            the character set to handle (may contain ranges)
     * @param type
     *            token type fro the characters ({@link Token#WHITESPACE} or {@link Token#SEPARATOR})
     * @param setIt
     *            if <code>true</code> the approbriate flags will be set, otherwise removed
     */
...
/**
 * Instances of this inner class are returned when a call to {@link TokenizerProperties#getProperties}. Each element of the enumeration contains a {@link TokenizerProperty} element.
 */
final class FullIterator implements Iterator {
    ...
    /**
     * constructor taking the calling {@link TokenizerProperties} object to retrieve the members holding {@link TokenizerProperty} elements which are iterated by this <code>FullIterator</code>
     * instance.
     * 
     * @param caseSensitiveMap
     *            map with properties where case matters
     * @param caseSensitiveMap
     *            map with properties where case doesn't matter
     */
    ...
    /**
     * Test wether there is another element in the iterated set or not. See {@link java.util.Iterator} for details.
     * 
     * @return <code>true</code>if another call to {@link #next} will return an object, <code>false</code> otherwise
     */
    @Override
    ...
    /**
     * Retrieve the next element in the iterated set. See {@link java.util.Iterator} for details.
     * 
...
/**
 * Instances of this inner class are returned when a call to {@link TokenizerProperties#getKeywords} or {@link TokenizerProperties#getPatterns}. Each element of the enumeration contains a
 * {@link TokenizerProperty} element, that in turn has the keyword or a pattern with its companion
 */
final class MapIterator implements Iterator {
    ...
    /**
     * constructor taking the a case-sensitive and a case-insensitive {@link java.util.Map} which are iterated by this <code>MapIterator</code> instance.
     * 
     * @param caseSensitiveMap
     *            map with properties where case matters
     * @param caseSensitiveMap
     ...
     * 
     * @return <code>true</code> if there are more {@link TokenizerProperty} elements, <code>false</code> otherwise
     */
    @Override
    public boolean hasNext() {
     ...
     *             if there is no more element in this iterator
     */
    @Override
    public Object next() {
        if (!hasNext()) {
     ...
     *             if {@link #next} has not been called before or <code>remove</code> has been called already after the last <code>next</code>.
     */
    @Override
    public void remove() {
        synchronized (this) {
...
/**
 * Iterator for comments, strings and special sequences. Instances of this inner class are returned when a call to one of the methods
 * <ul>
 * <li>
 * {@link #getBlockComments}</li>
 * <li>
 * {@link #getLineComments}</li>
 * <li>
 * {@link #getStrings}</li>
 * <li>
 * {@link #getSpecialSequences}</li>
 * </ul>
 * is done. Each element of the enumeration contains a {@link TokenizerProperty} element, that in turn has the comment, special sequence etc. together with its companion
 */
final class SpecialSequencesIterator implements Iterator {
    ...
    /**
     * constructor taking the calling <code>Tokenizer</code> and the type of the {@link TokenizerProperty}. If the type is 0 then special sequences, line and block comments are returned in one
     * iterator
     * 
     * @param parent
     *            the calling tokenizer
     * @param stores
     *            which array of {@link de.susebox.jtopas.impl.SequenceStore} to use
     * @param type
     *            type of the <code>TokenizerProperty</code>
     */
    ...
    /**
     * Remove the current special sequence entry from the collection. This is an alternative to {@link Tokenizer#removeSpecialSequence}.
     * 
     * @throws IllegalStateExcpetion
     *             if {@link #next} has not been called before or <code>remove</code> has been called already after the last <code>next</code>.
     */
