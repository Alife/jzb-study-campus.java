 ...
 * <ul>
 * <li>
 * a method implements a certain interface method that allows for a specific throwable like IOException, but the method itself may encounter a different throwable type like SQLException (wrapped
 * throwable)</li>
 * <li>
 * one application layer catches an throwable only to add its specific information in form of another throwable (throwable stack, nested throwable like in SQLException or MessagingException).</li>
 * </ul>
 * </p>
 * <p>
 * We provide the expected code in block comments starting with --&gt;, terminated by --&gt;. Note that the provided code also includes a new implementation of the base class method
 * {@link java.lang.Throwable#getMessage} using the text formatting capabilities of {@link java.text.MessageFormat}.
 * <p>
 * </p>
 * <strong>Note:</strong> This interface replaces the formerly used <code>ExceptionList</code> interface.
 * <p>
 * </p>
 * <strong>Note:</strong> With JDK 1.4, the <i>chained exception facility</i> is available, that implements a Throwable stack (or list of causes) already in {@link java.lang.Throwable}. Still, the
 * notion of a wrapped exception and the delayed formatting is a reason for this interface. </p>
 * 
...
public interface ThrowableList {
    ...
    /**
     * Getting the format string of a throwable message. This can also be the message itself if there are no arguments.
     * 
    ...
    /**
     * Check if <code>this</code> is only a throwable that wraps the real one. This might be nessecary to pass an throwable incompatible to a method declaration.
     * 
     * @return <code>true</code> if this is a wrapper throwable, <code>false</code> otherwise
     */
    ...
    /**
     * Method to traverse the throwable list. By convention, <code>nextThrowable</code>> returns the "earlier" throwable. By walking down the throwable list one gets the the following meaning: <br>
     * this happened because nextThrowable happened because nextThrowable happened... <br>
     * The next throwable has usually one of the following meaning: <br>
     * <ul>
     * <li>
     * It is the "real" throwable. An interface implementation might be allowed to throw only <code>IOException</code>, but actually has to pass on a <code>SQLException</code>. That could be done by
     * wrapping the <code>SQLException</code> into the <code>IOException</code>.</li>
     * <li>
     * The next throwable is "deeper" cause of this one (often called a nested throwable). A file couldn't be read in the first place and therefore not be attached to a mail. Both this throwable and
     * the one nested inside have their own message.</li></li> There are more than one basic throwable to be propagated. A simple parser might return all syntax errors in one throwable list. </li>
     * </ul>
     * 
    ...
    /**
     * If one likes to add ones own information to an throwable, this constructor is the easiest way to do so. By using such an approach a throwable trace with useful additional informations (which
    ...
    /**
     * This is the most complex way to construct an <code>ThrowableList</code>- Throwable.<br>
