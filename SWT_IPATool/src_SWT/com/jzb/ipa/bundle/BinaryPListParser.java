 ...
 * 
 * Copyright (c) 2005 Werner Randelshofer Staldenmattweg 2, Immensee, CH-6405, Switzerland. All rights reserved.
 * 
 * This software is the confidential and proprietary information of Werner Randelshofer. ("Confidential Information"). You shall not disclose such Confidential Information and shall use it only in
 * accordance with the terms of the license agreement you entered into with Werner Randelshofer.
 */
 ...
 * documentation</a>:
 * <p/>
 *"A property list is a data representation used by Mac OS X Cocoa and Core Foundation as a convenient way to store, organize, and access standard object types. Frequently called a ?plist,? a
 * property list is an object of one of several certain Cocoa or Core Foundation types, including arrays, dictionaries, strings, binary data, numbers, dates, and Boolean values. If the object is a
 * container (an array or dictionary), all objects contained within it must also be supported property list objects. (Arrays and dictionaries can contain objects not supported by the architecture, but
 * are then not property lists, and cannot be saved and restored with the various property list methods.)"
 * <p/>
...
System.out.println(root);
         ...
         return db.parse(new InputSource(new StringReader(root.toString())));
    }
    ...
    private void parseDate(DataInputStream in) throws IOException {
        ...
        throw new IOException("Date type is not supported yet!");
    }
